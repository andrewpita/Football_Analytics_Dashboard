...
plot.team=function(teams,ptype,ps,full){
  ...
  if (full == "Full Game") {
    ...
    if (ptype == "xSP") {
      ...
      teams.data %>% 
        select(team1.xSP,team2.xSP,playmins) %>% 
        mutate(team1.xSP=cumsum(team1.xSP),team2.xSP=cumsum(team2.xSP)) %>% 
        gather(key=team, value=value, - playmins) %>% 
        
        ggplot(aes(x=playmins/60,y=value,col=team)) + 
        geom_line() + 
        scale_color_manual(name="Team",labels=c(team1,team2),
                           values=c("Orange","Black")) +
        labs(title=paste0("(",match.info$time[1],"%) ",
                          team1," ",match.info$goals[1],"-",
                          match.info$goals[2]," ",team2, 
                          " (",match.info$time[2],"%)"), 
             subtitle="Possession shown in brackets",x="Playing minutes", 
             y="Net Possession Shot Potential", caption="Excluding penalties, own goals and big mistakes") + 
        theme(plot.title = element_text(hjust = 0.5),plot.subtitle=element_text(face="italic")) 
      
    } else if (ptype == "xGP") {
      
      
      teams.data %>% 
        select(team1.xGP,team2.xGP,playmins) %>% 
        ...
        labs(title=paste0("(",match.info$time[1],"%) ",
                          ...
                          " (",match.info$time[2],"%)"), 
             subtitle="Possession shown in brackets",x="Playing minutes", 
             y="Net Possession Goal Potential", caption="Excluding penalties, own goals and big mistakes") + 
        theme(plot.title = element_text(hjust = 0.5),plot.subtitle=element_text(face="italic")) 
...
pitch.plot = function(team, ps, full) {
...
shinyServer(function(input, output) {
  ...
  full = reactive({input$fullGame})
  
  

  output$plots = renderPlot({
    
    xgp = plot.team(c(home_team(),away_team()),ptype(),poss_tracker$poss_num, full())
    
    team1 = pitch.plot(home_team(), poss_tracker$poss_num, full())
    
    team2 = pitch.plot(away_team(), poss_tracker$poss_num, full())
    
    
    grid.arrange(grobs = list(xgp,team1, team2), ncols = 3, widths = c(2,1,1))
